# the Courier's Exchange

Token: `{{LOC:COURIERS_EXCHANGE}}`
Required file: `loc-couriers-exchange.png`

Use `loc-couriers-exchange.png` as the required visual reference whenever this location appears. Preserve the exact layout, palette and construction. 

**Status: NOT LOCKED — no canonical image yet.**

## What this has to hold

Where dispatch work is handed out. Window light on a working table. t1 spread 1.
...
1. Generate the canonical image. One figure, plain
   ...
   a plate.
2. Save it in this folder as exactly `loc-couriers-exchange.png`.
3. Paste the description into `block` for `COURIERS_EXCHANGE` in
