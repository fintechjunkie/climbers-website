# the Tower ground plaza

Token: `{{LOC:TOWER_PLAZA}}`
Required file: `loc-tower-plaza.png`

Use `loc-tower-plaza.png` as the required visual reference whenever this location appears. Preserve the exact layout, palette and construction. 

**Status: NOT LOCKED — no canonical image yet.**

## What this has to hold

Holds three hundred on a quiet week. The registration wall at the front. Screens on every building face carrying the registration feed. p3 spreads 7-18.
...
1. Generate the canonical image. One figure, plain
   ...
   a plate.
2. Save it in this folder as exactly `loc-tower-plaza.png`.
3. Paste the description into `block` for `TOWER_PLAZA` in
