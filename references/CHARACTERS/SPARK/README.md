# Spark

Token: `{{CHAR:SPARK}}`
Required file: `char-spark-canonical.png`

Use `char-spark-canonical.png` as the required visual reference whenever this figure appears. Preserve the exact face, silhouette, proportions and coloration. Clothing may change only where the wardrobe rows below permit it.

**Status: LOCKED.**

## What this has to hold

Human, green and gold. Mechanical RIGHT arm with fresh tool marks at the elbow, integrated deep enough that the shoulder sits differently from the left. Shakes with the mechanical hand by habit. Easy energy of someone whose mind is already made up.

## How to lock this

1. Generate the canonical image. One figure, full body, neutral pose, plain
   ground, nothing from a story beat in frame — this is a reference sheet, not
   a plate.
2. Save it in this folder as exactly `char-spark-canonical.png`.
3. Paste the description into `block` for `SPARK` in
   `content/roster.json`, and set `locked` to `true`.
4. Run `npm run prompts`. It will start attaching this file to every prompt
   that uses the token.

## Deviations from the block as written, and what was decided about each

_Record here anything the generated image does differently from the description,
and whether it was kept. Millbrook's reference folders do this and it is the
most useful part of them: the image is the authority, so when the two disagree
the description is what has to change._
