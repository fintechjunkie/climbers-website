...
Token: `{{CHAR:SERAPH}}`
Required file: `char-seraph-canonical.png`

Use `char-seraph-canonical.png` as the required visual reference whenever this figure appears. Preserve the exact face, silhouette, proportions and coloration. Clothing may change only where the wardrobe rows below permit it.

**Status: NOT LOCKED — no canonical image yet.**

## What this has to hold

Rotating polyhedrons of white and gold light. NEVER a face, never a body — 'almost like a face' means suggestion only. Beautiful the way an equation is beautiful.
...
1. Generate the canonical image. One figure, full body, neutral pose, plain
   ...
   a plate.
2. Save it in this folder as exactly `char-seraph-canonical.png`.
3. Paste the description into `block` for `SERAPH` in
