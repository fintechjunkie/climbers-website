...
export const type = {
  ...
  /* A part break inside the prose.
   *
   * Distinct from `kicker`, which is furniture — folios, labels, the running
   * head. A part break is the one piece of structure the author put INTO the
   * story, and at the kicker's 10.5px ceiling it came out at half the size of
   * the body text it was introducing, reading as a caption rather than as a
   * door into the next movement.
   *
   * Sized in cqh like the body so the two keep their relationship at every
   * book size, and pitched one step above it: 2.6 against the body's 2.02 is
   * about a 29% step, which is enough to register as a level of hierarchy
   * without the display face starting to shout.
   */
  sectionHead: 'clamp(15px, 2.6cqh, 25px)',
};
